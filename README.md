...
- **切换账号**：App 左下角有"⚙"设置齿轮按钮 → 点开后选"切换账号" → 彻底清除本地登录数据（cookie + localStorage + WebStorage）回到登录页，重新登录即切换到其他账号。
- **返回键行为**：站内先返回上一级（SPA hash 路由由 App 维护导航栈），回到首页后再按一次返回键才退出应用（2 秒内连按两次）。
  - 相关代码：`android/app/src/main/java/com/hynu/jiaowu/MainActivity.java`（设置齿轮按钮、返回键导航栈）。
